...
export class ManageEmployeesComponent implements OnInit {
  ...
  fetchUsers(): void {
    this.http.get<any>('http://localhost:8081/api/v1/auth/users')
      .subscribe({
        next: res => {
  ...
  fetchDepartments(): void {
    this.http.get<any>('http://localhost:8081/api/v1/auth/departments')
      .subscribe({
  ...
  confirmRoleChange(user: any): void {
    const body = { role: user.role.toUpperCase() }; // match Java enum
    this.http.put(`http://localhost:8081/api/v1/auth/users/${user.id}/role`, body)
      .subscribe({
        next: () => {
  ...
  confirmDepartmentChange(user: any): void {
    ...
    this.http.put(`http://localhost:8081/api/v1/auth/users/${user.id}/department`, body)
      .subscribe(() => {
  ...
  viewBalance(user: any): void {
    ...
    this.http.get<any>(`http://localhost:8082/api/v1/leave/admin/leave/balance/${user.id}`, headers)
      .subscribe({
